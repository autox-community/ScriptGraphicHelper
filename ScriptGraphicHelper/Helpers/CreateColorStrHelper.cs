...
using ScriptGraphicHelper.Utils.Engine;
...
namespace ScriptGraphicHelper.Helpers
{
    ...
    {
        public const string ATFindStr = "AT找色";
        public const string ATCmpStr = "AT比色";
        public const string ATAnchorFindStr = "AT锚点找色";
        public const string ATAnchorCmpStr = "AT锚点比色";
        public const string AnchorFindStr = "锚点找色";
        public const string AnchorCmpStr = "锚点比色";
        public const string AutojsFindStr = "AutoJs找色";
        public const string AutojsCmpStr = "AutoJs比色";
        public const string DMFindStr = "大漠找色";
        public const string AnjianFindStr = "按键找色";
        public const string AnjianCmpStr = "按键比色";
        //public const string DiyFindStr = "自定义找色";
        //public const string DiyCmpStr = "自定义比色";
        public const string CDFindStr = "触动找色";
        public const string CDCmpStr = "触动比色";
        public const string ECFindStr = "EC找色";
        public const string ECCmpStr = "EC比色";
    ...
    {
        public static string Create(string mode, ObservableCollection<ColorInfo> colorInfos, MyRange? rect = null)
        {
            ...
            try
            {
                return mode switch
                {
                    ...
                    FormatMode.DMFindStr => DmFindStr(list, rect),
                    FormatMode.ATCmpStr => AstatorCompareStr(list),
                    FormatMode.ATFindStr => AstatorFindStr(list, rect),
                    FormatMode.ATAnchorCmpStr => AstatorAnchorCompareStr(list),
                    FormatMode.ATAnchorFindStr => AstatorAnchorFindStr(list, rect),
                    FormatMode.AnchorCmpStr => AnchorCompareStr(list),
                    FormatMode.AnchorFindStr => AnchorFindStr(list, rect),
                    FormatMode.AnjianFindStr => AnjianFindStr(list, rect),
                    FormatMode.AnjianCmpStr => AnjianCompareStr(list),
                    FormatMode.CDFindStr => CdFindStr(list, rect),
                    FormatMode.CDCmpStr => CdCompareStr(list),
                    FormatMode.ECFindStr => EcFindStr(list, rect),
                    FormatMode.ECCmpStr => EcCompareStr(list),
        ...
        /// <param name="colorInfos"></param>
        /// <param name="rect"></param>
        /// <returns></returns>
        private static string DiyFormatStr(string mode, List<ColorInfo> colorInfos, MyRange rect)
        {
            var format = FormatConfig.GetFormat(mode);

            if (format.IsDiyFormat is true && format.DiyFormatFileName != null)
            {
                // 获取 diyFormatFileName 属性对应的值 (文件名)
                var fileName = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "MyFiles", format.DiyFormatFileName);
        ...
        private static string DiyCompareStr_Script(string fileName, List<ColorInfo> colorInfos)
        {
            ...
            engine.LoadScript(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "MyFiles", fileName));
            ...
            if (!compile.Success)
            {
                ...
                MessageBoxWindow.ShowAsync("编译失败:\r\n" + string.Join("\r\n", errorMessage.ToArray()));
                return string.Empty;
            }
        ...
        /// <param name="rect"></param>
        /// <returns></returns>
        private static string DiyFindStr_Script(string fileName, List<ColorInfo> colorInfos, MyRange rect)
        {
            var engine = new ScriptEngine();
            engine.LoadScript(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "MyFiles", fileName));
        ...
        {
            var sr = File.OpenText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "MyFiles", fileName));
            ...
            if (format == null)
            {
                MessageBoxWindow.ShowAsync("自定义格式错误!");
            }
        ...
        /// <summary>
        /// 自定义找色字符串 (将字符串中的变量,替换为数据)
        /// </summary>
        /// <param name="fileName"></param>
        /// <param name="colorInfos"></param>
        /// <param name="rect"></param>
        /// <returns></returns>
        private static string DiyFindStr_Json(string fileName, List<ColorInfo> colorInfos, MyRange rect)
        {
            var diyFormat = GetDiyFormatJson(fileName);
            var isInit = false;
        ...
        /// <param name="rect"></param>
        /// <returns></returns>
        private static string DmFindStr(List<ColorInfo> colorInfos, MyRange rect)
        {
        ...
        public static string CdFindStr(List<ColorInfo> colorInfos, MyRange rect)
        {
            var result = string.Empty;
            Point firstPoint = new();
            foreach (var colorInfo in colorInfos)
        ...
        private static string AnjianFindStr(List<ColorInfo> colorInfos, MyRange rect)
        {
            var result = string.Empty;
            var inited = false;
            Point firstPoint = new();
        ...
        private static string AutojsFindStr(List<ColorInfo> colorInfos, MyRange rect)
        {
            var result = string.Empty;
            Point firstPoint = new();
            foreach (var colorInfo in colorInfos)
        ...
        private static string EcFindStr(List<ColorInfo> colorInfos, MyRange rect)
        {
            var result = string.Empty;
            Point firstPoint = new();
            foreach (var colorInfo in colorInfos)
        ...
        private static string AnjianCompareStr(List<ColorInfo> colorInfos)
        {
            var result = "\"";
            if (Settings.Instance.AddInfo)
            {
        ...
        private static string CdCompareStr(List<ColorInfo> colorInfos)
        {
            var result = "{";
            foreach (var colorInfo in colorInfos)
            {
        ...
        private static string EcCompareStr(List<ColorInfo> colorInfos)
        {
            var result = "\"";
            foreach (var colorInfo in colorInfos)
            {
        ...
        private static string AnchorFindStr(List<ColorInfo> colorInfos, MyRange rect)
        {
            var result = "[" + ColorInfo.Width.ToString() + "," + ColorInfo.Height.ToString();
            if (Settings.Instance.AddRange)
            {
        ...
        private static string AstatorFindStr(List<ColorInfo> colorInfos, MyRange rect)
        {
            var result = string.Empty;
            if (Settings.Instance.AddRange)
            {
        ...
        private static string AstatorAnchorFindStr(List<ColorInfo> colorInfos, MyRange rect)
        {
            var result = string.Empty;
            if (Settings.Instance.AddRange)
            {
